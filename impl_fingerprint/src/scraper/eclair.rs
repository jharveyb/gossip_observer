...
mod tests {
    ...
    fn policy_defaults_stable() {
        for r in records() {
            ...
            assert_eq!(p.cltv_expiry_delta, Some(144), "{}", r.version);
            assert_eq!(p.fee_base_msat, Some(1000), "{}", r.version);
            assert_eq!(p.fee_proportional_millionths, Some(200), "{}", r.version);
    ...
    #[test]
    fn v090_hex_length() {
        // Highest bit is 47 → ceil(48/8) = 6 bytes → 12 hex chars.
        let hex = bits_to_hex(BITS_V090);
        assert_eq!(hex.len(), 12, "expected 6-byte hex, got: {hex}");
    }

    #[test]
    fn v0100_hex_length() {
        // Highest bit is 47 → ceil(48/8) = 6 bytes → 12 hex chars.
        let hex = bits_to_hex(BITS_V0100);
        assert_eq!(hex.len(), 12, "expected 6-byte hex, got: {hex}");
    }

    #[test]
    fn v090_v0100_differ_in_hex() {
        let recs = records();
        let v090 = recs.iter().find(|r| r.version == "v0.9.0").unwrap();
        let v0100 = recs.iter().find(|r| r.version == "v0.10.0").unwrap();
        assert_ne!(v090.node_feature_hex, v0100.node_feature_hex);
    }

    #[test]
    fn v090_data_loss_protect_optional() {
        let r = records().into_iter().find(|r| r.version == "v0.9.0").unwrap();
        let dlp = r.node_features.iter().find(|f| f.name == "data-loss-protect").unwrap();
        assert_eq!(dlp.requirement, FeatureRequirement::Optional);
    }

    #[test]
    fn v0100_data_loss_protect_mandatory() {
        let r = records().into_iter().find(|r| r.version == "v0.10.0").unwrap();
        let dlp = r.node_features.iter().find(|f| f.name == "data-loss-protect").unwrap();
        assert_eq!(dlp.requirement, FeatureRequirement::Mandatory);
    }

    #[test]
    fn v090_static_remote_key_optional() {
        let r = records().into_iter().find(|r| r.version == "v0.9.0").unwrap();
    ...
    #[test]
    fn v0100_static_remote_key_mandatory() {
        let r = records().into_iter().find(|r| r.version == "v0.10.0").unwrap();
        let srk = r.node_features.iter().find(|f| f.name == "static-remote-key").unwrap();
    ...
    #[test]
    fn v0100_has_dual_fund() {
        let r = records().into_iter().find(|r| r.version == "v0.10.0").unwrap();
        assert!(r.node_features.iter().any(|f| f.name == "dual-fund"));
    }

    #[test]
    fn v090_lacks_dual_fund() {
        let r = records().into_iter().find(|r| r.version == "v0.9.0").unwrap();
        assert!(!r.node_features.iter().any(|f| f.name == "dual-fund"));
    }

    #[test]
    fn eclair_has_onion_messages() {
        for r in records() {
            assert!(
    ...
    fn hex_bit_round_trip() {
        ...
        let mut expected: Vec<u16> = BITS_V090.to_vec();
        expected.sort_unstable();
        assert_eq!(hex_to_bits(&bits_to_hex(BITS_V090)), expected, "v0.9.0");

        let mut expected: Vec<u16> = BITS_V0100.to_vec();
        expected.sort_unstable();
        assert_eq!(hex_to_bits(&bits_to_hex(BITS_V0100)), expected, "v0.10.0");
    }
}
